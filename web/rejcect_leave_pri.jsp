...
<html>
    ...
    <body>
        ...
        conn = DB.dbcon.connectDB_con();
         pst = conn.prepareStatement("INSERT INTO `reject_leave_principle`(`id`, `department`, `email`, `leave_type`, `To`, `from`,`days`, `description`, `doc_name`, `allocated_cl`, `taken_cl`, `pending_cl`,`accept`,`pri_accept`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
//                    response.sendRedirect("add_details.jsp");
  ...
  String sql = "DELETE FROM  `accept_leave_hod` "
            ...
            boolean sendmail=main.sendconfirmationmailfromPRINCIPLE_Reject(To,from,email);
             if (!sendmail && rs > 0) {
