...
<html>
    ...
    <body>
        ...
        conn = DB.dbcon.connectDB_con();
         pst = conn.prepareStatement("INSERT INTO `accept_leave_hod`(`id`, `department`, `email`, `leave_type`, `To`, `from`,`days`, `description`, `doc_name`, `allocated_cl`, `taken_cl`, `pending_cl`,`accept`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)");
...
//                    response.sendRedirect("add_details.jsp");
  ...
  String sql = "DELETE FROM  `"+tablename+"` "
            ...
            int rs = pst.executeUpdate(sql);
             boolean sendmail=main.sendconfirmationmailfromHOD(To,from,email);
             if (!sendmail && rs > 0) {
                System.out.println("file is deleted");
                       session.setAttribute("flash_message", "Request Sent to OS");
                   session.setAttribute("flash_type", "success");
